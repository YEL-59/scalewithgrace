...
const ResumeBuilderStepper = () => {
  const { setFormData } = useResume();
  const location = useLocation();
  const generatedResume = location.state?.generatedResume; // <-- from upload step

  const passedData = location.state?.resumeData || null; // ✅ from ResumeUploadPrompt
  const resumeId = location.state?.resumeId || null;
  const generatedSummary =
    location?.state?.summary || localStorage.getItem("resumeSummary") || "";
  const { data: resumeDataFromApi, isLoading } = useResumeById(resumeId, {
    enabled: !passedData, // skip fetch if passed data is available
  });

  const resumeData = passedData || resumeDataFromApi || null;
  ...
  // ✅ Auto-fill form when resumeData changes
  useEffect(() => {
    if (generatedResume) {
      ...
      methods.reset({
        ...
        state: "", // not provided in AI JSON
        city: "", // not provided in AI JSON

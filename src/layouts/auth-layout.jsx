...
import { useGetLoginStaticInfo } from "@/hooks/auth.hook";

export default function AuthLayout() {
  useScrollToTop();
  const { signininfostatic, isLoading } = useGetLoginStaticInfo();
  return (
    <div className="h-screen grid grid-cols-1 lg:grid-cols-2">
      ...
      <div className="hidden lg:flex bg-[#EDFCFF] rounded-l-[20px] relative items-end justify-center overflow-hidden">
        ...
        {!isLoading && signininfostatic?.image && (
          <img
            src={signininfostatic?.image}
            alt={signininfostatic?.title || "Login Illustration"}
            className="absolute top-0 right-0 w-full h-4/5 object-cover z-10"
          />
        )}
        ...
        <div className="absolute bottom-6 left-6 z-50">
          <h1 className="font-medium text-[#1E1E1E] text-[22px] xl:text-[26px]">
            {signininfostatic?.title || " Sign in to Karially "}
          </h1>
          <p className="text-[#959595] text-base xl:text-sm">
            {signininfostatic?.description ||
